...
export default function DoctorDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setDoctorProfile(profileRes.data.data.doctor);
    } catch (error) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Welcome, Dr. {doctorProfile?.user?.name}</h1>
        ...
        {doctorProfile?.status !== 'verified' && (
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-800 text-sm font-medium flex items-center gap-2">
            ...
            Your account is currently {doctorProfile?.status}. You cannot accept appointments until verified by an admin.
          </div>
        )}
      </div>
