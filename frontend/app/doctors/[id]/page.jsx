    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          {/* Header Card */}
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] border-0 shadow-sm p-8 lg:p-10 flex flex-col md:flex-row gap-8">
             ...
             <div>
               <div className="flex items-center gap-3 mb-2">
                 <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white">Dr. {doctor.user?.name}</h1>
                 <span className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1.5">
               ...
               <div className="flex flex-wrap gap-4 text-sm font-medium text-gray-600 dark:text-gray-400">
                 <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-[1rem]">
                 ...
                 <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-[1rem]">
                   ...
                   <span className="font-bold text-gray-900 dark:text-white">{doctor.rating > 0 ? doctor.rating.toFixed(1) : 'New'}</span> ({doctor.totalReviews} reviews)
                 </div>
               </div>
               ...
               {doctor.qualifications && doctor.qualifications.length > 0 && (
                 <div className="mt-6 flex flex-wrap gap-2">
                   {doctor.qualifications.map(q => (
                     <span key={q} className="px-3 py-1.5 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-[1rem] uppercase tracking-wider">{q}</span>
                   ))}
                 </div>
               )}
             </div>
          ...
          {doctor.bio && (
            <div className="bg-white dark:bg-gray-900 rounded-[2rem] border-0 shadow-sm p-8 lg:p-10">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">About Doctor</h2>
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed font-medium">{doctor.bio}</p>
            </div>
          )}

          {/* Reviews */}
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] border-0 shadow-sm p-8 lg:p-10">
             <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Patient Reviews</h2>
             {reviews.length > 0 ? (
               <div className="space-y-6">
                 {reviews.map(review => (
                   <div key={review._id} className="border-b border-gray-100 dark:border-gray-800 last:border-0 pb-6 last:pb-0">
                     <div className="flex items-center gap-3 mb-3">
                       <div className="flex text-yellow-400 bg-gray-50 dark:bg-gray-800 px-2 py-1 rounded-[1rem]">
                         {[...Array(5)].map((_, i) => (
                       ...
                       <span className="text-sm font-bold text-gray-900 dark:text-white">{review.patient?.name}</span>
                       <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 ml-auto">{new Date(review.createdAt).toLocaleDateString()}</span>
                     </div>
                     {review.comment && <p className="text-gray-600 dark:text-gray-400 text-sm mt-2 font-medium">{review.comment}</p>}
                   </div>
                 ))}
               </div>
             ) : (
               <p className="text-gray-500 dark:text-gray-400 italic font-medium">No reviews yet.</p>
             )}
          </div>
        </div>
        ...
        <div className="w-full lg:w-[400px] shrink-0">
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] border-0 shadow-lg sticky top-8 p-8 lg:p-10">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Book Appointment</h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm font-bold mb-8 uppercase tracking-wider">Consultation Fee: <span className="text-primary-600 dark:text-primary-400 text-xl">₹{doctor.consultationFee}</span></p>
            ...
            <form onSubmit={handleBook} className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-4 rounded-[1.5rem] border-0 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 outline-none resize-none transition-all"
                />
              </div>
